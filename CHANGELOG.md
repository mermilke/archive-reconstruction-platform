...
- **Mixed-format example archive** (`examples/archive/`, generated by
  `scripts/generate_example_archive.py`) — a realistic 14-file pile that mixes
  `.txt`, `.eml`, `.mbox`, and `.pdf` and dedups all three thread formats
  together: an `.eml` flagged as a subset of a `.txt` thread, an `.mbox`
  collapsed as a timezone-shifted duplicate, and a forwarded `.eml` kept for its
  unique PDF attachment. PDFs ride along as attachments (matched by name), so the
  engine still parses only `.txt`/`.eml`/`.mbox` and stays zero-dependency.
  Covered by `tests/test_archive_example.py`.
